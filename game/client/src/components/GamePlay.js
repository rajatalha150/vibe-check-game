...
const PowerUpSection = styled.div`
  background: #f1c40f;
  color: #333;
  padding: 15px;
  border-radius: 15px;
...
function GamePlay({ gameState, scenario, responses, onSubmitResponse, onUsePowerUp, playerId }) {
  ...
  return (
    ...
    >
      ...
      {powerUp && (
        <PowerUpSection>
          <strong>Your Power-up: {powerUp.name}</strong>
          <p>{powerUp.description}</p>
          <PowerUpButton onClick={handleUsePowerUp} disabled={powerUp.used}>
            {powerUp.used ? 'Used' : 'Use Power-up'}
          </PowerUpButton>
        </PowerUpSection>
      )}
